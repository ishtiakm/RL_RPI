# Reinforcement Learning ---- HW5 -----
# This code was executed in python 3.9
# This is the cliff MDP problem

#To execute the program, go to the folder directory and execute the following
python3 cliff_main.py


#The rewuired packages are listed in environment.yml

##############
To Automate the whole environment setup and execution. Just open up the terminal, go to the project folder and type following:
setup_and_execute.sh


#This will automatically make the COnda environment, install packages and execute the program
